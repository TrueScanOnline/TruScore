...
1. **Governed-only public Signals** â€” `buildProductScanResult` default `phase6SignalSourceMode` changed from `transitional` â†’ `governed_5b_only`. Omission cannot restore legacy/synthetic cards. Live Result screen passes `governed_5b_only` explicitly.
2. **Non-governed producers unreachable** â€” Limited Product Data, Web Search Source, and preference Class C banners only emit when `phase6SignalSourceMode: 'transitional'` (controlled tests). Coverage flags remain internal.
3. **KitKat AU-02** â€” reviewed alias/canonical **exact token** match on `product_name`/`generic_name`; prefer name-token hits over brands-only umbrella so NestlÃ©-only OFF brands + KitKat title â†’ **B0060** â†’ `SIG_NEWS_GLOBAL_001`. Preserves Cadbury dual-positive, Ritz/Philadelphia/Coca-Cola negatives. No A-data mutation, NestlÃ©-parent SL, force-hit, or synthetic GTIN.
4. **Safety assurance** â€” retained Stage 2 deterministic MILO/path-control tests; no manufactured on-device recall.
5. **No** Launch Corpus / Workstream Aâ€“B / TruScore changes. No Build 31 skeleton cycle.
...
| `src/__tests__/unit/workstreamC/resolveWorkstreamCRetailChain.test.ts` | KitKat + NestlÃ©-only controls |
...
See `build30_signal_producer_matrix.csv` â€” all synthetic/preference public producers **removed** from default path; governed Workstream C + Stage 2 MILO **retained**.
...
| KitKat NestlÃ©-only brands â†’ GLOBAL_001 | Yes |
| Cadbury dual / Ritz / Philadelphia negatives preserved | Yes |
| Closed A/B assets unchanged | Yes |
| TruScore scoring unchanged | Yes (Stage 2 + no score edits) |
...
- Umbrella public Signals heading still under founder consideration â€” not hardcoded.
...
1. Build 30 baseline â€” `build30_runtime_and_release_baseline.md`  
2. As-built inventory â€” `build30_as_built_signals_alerts_inventory.md`  
3. Producer matrix â€” `build30_signal_producer_matrix.csv`  
4. Subject scope + product_family assessment â€” `build30_subject_scope_and_product_family_assessment.md`  
