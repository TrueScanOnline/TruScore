...
**Remediation commit:** _(filled at commit)_  

## Implementation summary

1. **Governed-only public Signals** — `buildProductScanResult` default `phase6SignalSourceMode` changed from `transitional` → `governed_5b_only`. Omission cannot restore legacy/synthetic cards. Live Result screen passes `governed_5b_only` explicitly.
...
3. **KitKat AU-02** — reviewed alias/canonical **exact token** match on `product_name`/`generic_name`; prefer name-token hits over brands-only umbrella so Nestlé-only OFF brands + KitKat title → **B0060** → `SIG_NEWS_GLOBAL_001`. Preserves Cadbury dual-positive, Ritz/Philadelphia/Coca-Cola negatives. No A-data mutation, Nestlé-parent SL, force-hit, or synthetic GTIN.
...
| `src/__tests__/unit/workstreamC/resolveWorkstreamCRetailChain.test.ts` | KitKat + Nestlé-only controls |
...
| KitKat Nestlé-only brands → GLOBAL_001 | Yes |
