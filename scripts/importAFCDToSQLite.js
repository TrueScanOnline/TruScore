/**
 * Import AFCD (Australian Food Composition Database) Excel files into SQLite
 * This enables the app to query FSANZ by product name (not barcode)
 * 
 * Usage: node scripts/importAFCDToSQLite.js
 */
...
const EXCEL_FILE = path.join(__dirname, '..', 'Database files', 'AU Release 2 - Nutrient file.xlsx');
const DB_PATH = path.join(__dirname, '..', 'src', 'data', 'afcd.db');

console.log('========================================');
console.log('Import AFCD to SQLite');
console.log('========================================');
console.log('');
...
if (!fs.existsSync(EXCEL_FILE)) {
  ...
  console.error('Please ensure the AFCD Excel file is in the correct location.');
  process.exit(1);
}
...
try {
  ...
  // Create table
  db.exec(`
    CREATE TABLE IF NOT EXISTS afcd_foods (
    ...
    CREATE INDEX IF NOT EXISTS idx_food_name ON afcd_foods(food_name);
    CREATE INDEX IF NOT EXISTS idx_food_group ON afcd_foods(food_group);
    
    DELETE FROM afcd_foods;
  `);

  const insert = db.prepare(`
    INSERT INTO afcd_foods (
  ...
  const insertMany = db.transaction((rows) => {
    for (const row of rows) {
      insert.run(
        row.food_code || row['Food Code'] || null,
  ...
  console.error('Error importing AFCD:', error.error);
