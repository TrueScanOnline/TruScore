/**
 * Final CARE Pillar Test - Uses Actual Brand Database Logic
 * Tests barcodes and provides detailed breakdown with correct brand matching
 */
...
// Import actual brand database (simplified for Node.js)
// We'll check the actual brand names from the database
const FERRERO_BRANDS = ['ferrero', 'nutella', 'ferrero rocher', 'kinder'];
const NESTLE_BRANDS = ['nestle', 'nestlé', 'nescafe', 'nespresso', 'kit kat', 'smarties'];
const MARS_BRANDS = ['mars', 'm&m\'s', 'snickers', 'twix', 'milky way', 'skittles', 'starburst'];
const UNILEVER_BRANDS = ['unilever', 'dove', 'lipton', 'hellmann\'s', 'knorr', 'magnum', 'ben & jerry\'s'];
const P_G_BRANDS = ['procter & gamble', 'p&g', 'tide', 'ariel', 'pampers', 'crest', 'oral-b', 'gillette'];
...
function checkBrandForViolations(brandName: string): {
  ...
  // Check Ferrero
  if (FERRERO_BRANDS.some(fb => primaryBrand.includes(fb) || fb.includes(primaryBrand) || 
                                secondaryBrand.includes(fb) || fb.includes(secondaryBrand))) {
    hasLaborViolations = true;
    matchedBrand = 'Ferrero';
  }
  
  // Check Nestlé
  if (!hasLaborViolations && NESTLE_BRANDS.some(nb => 
    primaryBrand.includes(nb) || nb.includes(primaryBrand) || 
    secondaryBrand.includes(nb) || nb.includes(secondaryBrand))) {
    hasLaborViolations = true;
    matchedBrand = 'Nestlé';
  }
  
  // Check Mars
  if (!hasLaborViolations && MARS_BRANDS.some(mb => 
    primaryBrand.includes(mb) || mb.includes(primaryBrand) || 
    secondaryBrand.includes(mb) || mb.includes(secondaryBrand))) {
    hasLaborViolations = true;
    matchedBrand = 'Mars';
  }
  
  // Check for animal cruelty (Unilever, P&G)
  let hasAnimalCruelty = false;
  let matchedParent: string | null = null;
  
  if (UNILEVER_BRANDS.some(ub => 
    primaryBrand.includes(ub) || ub.includes(primaryBrand) || 
    secondaryBrand.includes(ub) || ub.includes(secondaryBrand))) {
    hasAnimalCruelty = true;
    matchedParent = 'Unilever';
  }
  
  if (!hasAnimalCruelty && P_G_BRANDS.some(pgb => 
    primaryBrand.includes(pgb) || pgb.includes(primaryBrand) || 
    secondaryBrand.includes(pgb) || pgb.includes(secondaryBrand))) {
    hasAnimalCruelty = true;
...
// Calculate CARE pillar with detailed tracking
function calculateCarePillarDetailed(product: Product): {
  ...
  dataSources: {
    labelsTags: { available: boolean; count: number; source: string; database: string };
    brands: { available: boolean; value: string; source: string; database: string };
    brandOwner: { available: boolean; value: string; source: string; database: string };
    recalls: { available: boolean; count: number; source: string; database: string };
  ...
  const dataSources = {
    labelsTags: {
      ...
      database: 'world.openfoodfacts.org (public API, no key required)',
    },
    brands: {
      available: !!brands,
      value: brands || 'N/A',
    ...
    recalls: {
      ...
      source: 'Recalls API',
      database: 'FDA/CFIA/RASFF (public APIs, no key required) - fetched BEFORE TruScore calculation',
    },
    brandDatabase: {
      available: true,
      matched: false,
...
async function main() {
  ...
  const adjustedPct = results.length > 0 ? ((adjustedProducts.length / results.length) * 100).toFixed(1) : '0.0';
  const defaultPct = results.length > 0 ? (((results.length - adjustedProducts.length) / results.length) * 100).toFixed(1) : '0.0';
  report.push(`Products with CARE Score Changes: ${adjustedProducts.length} (${adjustedPct}%)`);
  report.push(`Products with Default Score (15): ${results.length - adjustedProducts.length} (${defaultPct}%)`);
  report.push('');
  ...
  const reportPath = path.join(__dirname, '..', 'TruScore logic', 'care-pillar-score-changes-final.txt');
