...
if (-not $hasDatabase) {
    Write-Host ""
    Write-Host "[WARN] No database is configured!" -ForegroundColor Yellow
...
foreach ($var in $optionalVars) {
    if ($envText -match [regex]::Escape($var)) {
        ...
        Write-Host "[MISS] $var is NOT set (optional)" -ForegroundColor Yellow
    }
}

if (-not $hasPhotoStorage) {
    Write-Host ""
    Write-Host "[WARN] Photo storage is not fully configured (no BLOB or Cloudinary name)!" -ForegroundColor Yellow
...
if ($hasDatabase -and $hasPhotoStorage) {
    Write-Host "[OK] Backend is properly configured for production" -ForegroundColor Green
    exit 0
} elseif ($hasDatabase) {
    Write-Host "[WARN] Database is configured, but photo storage is missing" -ForegroundColor Yellow
    exit 0
} else {
    Write-Host "[FAIL] Database is NOT configured - CRITICAL for production" -ForegroundColor Red
    exit 1
}
