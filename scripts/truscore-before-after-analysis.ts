/**
 * Before/After TruScore Analysis Script
 * 
 * Compares TruScore calculations using old vs new pillar logic
 * for a list of barcodes and generates a comparison report
 */

import * as fs from 'fs';
import * as path from 'path';

// Type definitions (simplified to avoid React Native dependencies)
interface Product {
  barcode?: string;
  product_name?: string;
...
// Import the current (new) pillar calculators
// We'll need to copy the logic inline to avoid React Native dependencies
// For now, let's use a simplified approach

// OLD LOGIC - Body Pillar
function calculateBodyPillarOld(product: Product) {
...
function calculatePlanetPillarOld(product: Product) {
  let score = 15; // Base score
  ...
  // OLD Palm oil: Non-certified=-8, RSPO=0, brand overlay=-5
  if (product.palm_oil_analysis?.containsPalmOil && !product.palm_oil_analysis.isPalmOilFree) {
    ...
    // Brand overlay would be -5 (simplified for comparison)
  }
  
  // OLD Packaging: All recyclable=+5, some=+2, high eco-cost=-5
...
function calculateOpenPillarOld(product: Product) {
  let score = 15; // Base score
  ...
  // OLD Hidden terms: 1=-5, 2=-10, >=3=-15 (cap -20)
...
// OLD LOGIC - Calculate using old implementation
...
// Fetch product data directly from Open Food Facts API (standalone, no React Native deps)
...
function generateReport(results: any[]) {
  ...
  const bodyDiff = validResults.reduce((sum, r) => sum + r.difference.body, 0) / validResults.length;
  const planetDiff = validResults.reduce((sum, r) => sum + r.difference.planet, 0) / validResults.length;
  const openDiff = validResults.reduce((sum, r) => sum + r.difference.open, 0) / validResults.length;
  ...
  report.push(`Products with Increased Scores: ${increased} (${((increased / validResults.length) * 100).toFixed(1)}%)`);
  report.push(`Products with Decreased Scores: ${decreased} (${((decreased / validResults.length) * 100).toFixed(1)}%)`);
  report.push(`Products with Unchanged Scores: ${unchanged} (${((unchanged / validResults.length) * 100).toFixed(1)}%)`);
  report.push('');
