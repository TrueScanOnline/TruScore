 ...
 * - Base: 15 (uniform).
 * - BBFAW: BBFAW-specific candidates (`bbfawOwnerHint` first when present, then product brands);
 *   resolve product → brand → parent (alias map for identity only), then canonical BBFAW score table
 *   for Tier + Impact; apply when matched, else no BBFAW adjustment.
 * - KTC: KTC-specific candidates (`ktcOwnerHint` first when present, then product brands) — never
 *   the BBFAW owner hint; resolve product → brand → KTC parent, then benchmark bands; first match wins.
...
export function calculateEthicsPillar(
  ...
  const productCandidates = getEthicsCompanyCandidates(product);
  // Benchmark-specific candidate lists — BBFAW owner hint must not influence KTC (and vice versa).
  const bbfawCandidates = benchmarkCtx.bbfawOwnerHint
    ? [benchmarkCtx.bbfawOwnerHint, ...productCandidates]
    : productCandidates;
  const ktcCandidates = benchmarkCtx.ktcOwnerHint
    ? [benchmarkCtx.ktcOwnerHint, ...productCandidates]
    : productCandidates;

  pushAdjustment(adjustments, 'ethics-v37-base', 0, 'Base score (assumes ethical until poor ratings)');

  // Try each candidate: resolve via mapping (identity), then BBFAW canonical score lookup; first match wins.
  ...
  if (benchmarkCtx.benchmarkEligible) {
    ...
    // Official cycle score table is authoritative — mapping rows are identity/attribution only.
    tier = bbfawData?.tier ?? null;
    ...
    if (tier || impactRating) {
      ...
      // Same-cycle provenance for the commentary tokens from the firing governed record.
      const bbfawYear =
        benchmarkCtx.bbfawFrozen?.snapshot_ref.benchmark_cycle ?? bbfawData?.year;
  ...
  // KTC (KnowTheChain) 2026 – KTC-specific candidates only (no BBFAW owner hint)
  let ktcMatched: KTCParentData | null = null;
  let ktcScoreAdjustment = 0;
  if (benchmarkCtx.benchmarkEligible) {
    for (const raw of ktcCandidates) {
  ...
  if (ktcMatched && ktcScoreAdjustment !== 0 && ktcBandId) {
    const ktcYearRaw = resolveKtcGovernedBenchmarkYear(benchmarkCtx.ktcFrozen, ktcMatched.year);
    const ktcYear =
      ktcYearRaw != null && /^\d+$/.test(String(ktcYearRaw).trim())
        ? Number(String(ktcYearRaw).trim())
        : ktcYearRaw;
    const ktcYearLabel = ktcYear != null ? `KTC ${ktcYear}` : 'KTC';
    pushAdjustment(
      adjustments,
      ktcBandId,
      ktcScoreAdjustment,
