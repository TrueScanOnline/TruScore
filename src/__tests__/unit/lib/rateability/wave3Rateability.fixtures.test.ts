...
describe('Wave 3 Rateability / Confidence / NR — §16 fixtures', () => {
  ...
  describe('C — Claims', () => {
    ...
    it('ethics production path stamps completed_no_applicable_result when frozen-ineligible', () => {
      const product = baseProduct();
      // Attached frozen attribution disables ethics scoring; no identity → KTC materialize skipped
      (product as any)._frozen_benchmark_attribution = {
        eligibility: { ethics_scoring_eligible: false },
        subject_resolution: {},
        snapshot_ref: {},
      };
      const ethics = calculateEthicsPillar(product);
      ...
      for (const c of checks) {
        expect(c.not_applicable_resolution).toBe('completed_no_applicable_result');
      }
    });
