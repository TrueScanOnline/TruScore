...
async function fetchFromWoolworthsNZ(barcode: string): Promise<Product | null> {
  try {
    logger.debug(`[Woolworths NZ] Attempting to fetch product: ${barcode}`);
    ...
    let response = await fetch(searchUrl1, {
      headers: {
...
async function fetchFromFoodstuffs(barcode: string): Promise<Product | null> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const response = await fetch(paknsaveUrl, {
        method: 'POST',
        headers: {
      ...
      // If we get here, product not found
      return null;
    } catch (error) {
      ...
      if (attempt < maxRetries && (
        ...
        logger.debug(`Foodstuffs retry ${attempt}/${maxRetries} for ${barcode}`);
        await new Promise(resolve => setTimeout(resolve, 1000 * attempt));
        continue;
      }
 ...
 */
/**
 * Fetch product from NZ store APIs with improved error handling and retry logic
 * Only tries if user is in NZ (detected from device locale)
 */
export async function fetchProductFromNZStores(barcode: string, retryCount = 0): Promise<Product | null> {
  ...
  try {
    ...
    // If both failed and we haven't exceeded retry limit, retry once
    if (retryCount < MAX_RETRIES) {
      const errorMessage = 
        (woolworthsProduct.status === 'rejected' ? woolworthsProduct.reason : null) ||
        (foodstuffsProduct.status === 'rejected' ? foodstuffsProduct.reason : null);
      
      // Only retry on network errors, not on 404s
      if (errorMessage && !errorMessage.toString().includes('404')) {
        logger.debug(`NZ store API retry ${retryCount + 1}/${MAX_RETRIES} for ${barcode}`);
        // Wait a bit before retry (exponential backoff)
        await new Promise(resolve => setTimeout(resolve, 1000 * (retryCount + 1)));
        return fetchProductFromNZStores(barcode, retryCount + 1);
      }
    }
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.debug(`NZ store API error for ${barcode}:`, errorMessage);
    
    // Retry on network errors
    if (retryCount < MAX_RETRIES && !errorMessage.includes('404')) {
      logger.debug(`NZ store API retry ${retryCount + 1}/${MAX_RETRIES} for ${barcode}`);
      await new Promise(resolve => setTimeout(resolve, 1000 * (retryCount + 1)));
      return fetchProductFromNZStores(barcode, retryCount + 1);
    }
