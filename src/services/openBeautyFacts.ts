...
export async function fetchProductFromOBF(barcode: string): Promise<Product | null> {
  try {
    ...
    // Open Beauty Facts may return status: 0 with partial product data
    if (!data.product) {
    ...
    if (data.status === 0) {
      console.log(`OBF API returned status: 0 but product data exists for ${barcode} (accepting product)`);
    }
